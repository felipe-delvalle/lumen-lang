...
1. **Self-hosting. Done (2026).** The compiler is written in Lumen and reproduces itself byte-for-byte (`lumenc.lm`, `SELF: MATCH`, 29/29 conformance programs bit-identical per `seed/selfhost_diff.mjs`, verified live). It is a real language, not a demo.
2. **Native backend. Done (2026, the native fixpoint).** Speed stops being a disqualifier: Lumen-written C and LLVM emitters lower the IR to native code that matches or beats hand-written C, and a generation-2 compiler built from the pipeline's own output rebuilds byte-identically (`native/native_fixpoint_test.mjs`, in CI). The seed interpreter is not a throwaway bootstrap; it is kept forever as the reference oracle every native artifact is gated bit-identical against. A parallel campaign is narrowing the toolchain's own remaining non-Lumen dependency: the reproducible wasm-free C bootstrap (R1, `native/lumenc.bootstrap.c`, PR #62), the native-only compile-and-run backend and corpus parity gate (R2, PR #63), the emitter and optimizer C bootstraps (R4, PR #64), and the resident native compiler server (R3, PR #65) have landed; the full air-gap test (building and running the toolchain on a machine with nothing but Lumen and its named substrate) is not yet green.
3. **Batteries.** Standard library (starting with the number tower below), package manager, language server, formatter, debugger. Underway: structured diagnostics, `lumen fix`, the warm daemon, and the MCP surface already ship, and the HTTP protocol kernel already serves live traffic.
...
3. **The two number stories: reproducibility and exactness** (`RISKS` risk 4, decision D9). No serious quant, scientific, or financial adoption without a real number story, and the honest version is that there are two orthogonal ones. Reproducibility: the same computation yields the same bits across runs and across both backends (the determinism contract; it forbids fused-multiply-add and reassociation, the open D9 cost). Exactness: money and prices are decimal, so a coupon, a notional, or a settlement amount must be value-exact, not a binary-float approximation where `0.1 + 0.2` is not `0.3`. A deterministic float is bit-reproducible and still wrong for money; it satisfies reproducibility and still fails an auditor on exactness. IEEE `Float` already ships in the seed and in the self-hosted compiler, and D9 is the open reproducibility decision. The exactness axis is now partially landed: the seed ships a first-class exact-decimal `Dec` type (a `1.50d` literal, fixed scale, `0.1d + 0.2d == 0.3d` exactly, PR #68, velocity-ledger entry #1), which proved the governance half of the emblem in one day of wall-clock rather than the years a Steering-Council process takes, exactly as CPython's proposed `1.50d` predicts. What remains before the axis is closed: both native emitters, self-hosted-compiler parity, and the external Python-decimal oracle gate, each bit-identical. Non-negotiable for the beachhead.
...
- Land both number axes: the deterministic-float cost model (decision D9) and the exact-decimal money type. IEEE `Float` already ships in the seed and the self-hosted compiler; exact decimal landed in the seed as `Dec` (PR #68), and the emitters, selfhost parity, and the Python-decimal oracle gate close the axis. Verified finance is the pillar that needs it first.
- `Result`, `match`, and the non-coercing `?` operator already landed, and a verified quant kernel already runs (Black-Scholes to 10.4506, an exact discounted-cash-flow). Extend the catalog: curves, risk, exposure, each one proven and replayable.
- Keep every verified kernel proving its effects and replaying to the bit, so an agent writes it and the compiler certifies both what it touched and what it computed.
- Stand up the reinforcement-against-the-compiler loop early, even tiny; the warm daemon and the MCP surface that make it possible already ship, and the measurement rig itself exists (`bench/promptgreen/`, v0: ten frozen tasks, hidden tests never shown to the author, proven so far by a scripted deterministic author, not yet a real model or a published Lumen-versus-Python number).
...
## The campaign ledger: dimension, wave, arc, gate

The wedge above is prose; `bench/scoreboard.json` (rendered into `bench/DASHBOARD.md`) is the same commitment kept as a living, gated file. This is that ledger's build-plan slice: for each of the thirteen scored dimensions (fourteen counting the two generated-code-speed sub-axes), which campaign wave is currently assigned to it and which roadmap arc and gate would move it forward. Deliberately no status or verdict column here; the verdicts live in `VISION_2036.md` and `LANGUAGE_COMPARISON.md`, and this table would only go stale trying to restate them. A dash means neither doc has tagged that dimension with a wave or arc yet.

| # | Dimension | Wave | Arc | Gate |
|---|-----------|------|-----|------|
| 1 | Provable correctness | W7 | 2 | Contracts v0 and Capabilities v1 landing as CI-gated Arc 2 features rather than design documents |
| 2 | Numeric exactness (decimal) | W1 | 1 | `0.1d + 0.2d == 0.3` landing gated byte-identical across the interpreter and both native backends |
| 3 | Determinism and replay | - | 2 | `lumen run --record` landing with a deterministic tape, gated in CI |
| 4 | Effect safety and capability sandboxing | - | 2 | The purity gate and a tenant-isolation type-error test becoming CI gates, per the capabilities RFC |
| 5 | Toolchain trust (self-hosting fidelity) | - | - | Already true; no pending gate |
| 6 | Compiler as a reward environment | - | 3 | The RL-lift experiment running and publishing a measured authorship gain |
| 7a | Generated-code speed (runtime) | W6 | - | A native optimizer near LLVM/GCC parity (autovectorization, LTO, a numeric stdlib) landing |
| 7b | Compile latency (sub-axis) | W6 | - | A cross-language compile-latency comparison against the field being designed and run |
| 8 | Build determinism and supply chain | - | 1 | The full air-gap test passing |
| 9 | AI-authorability (intent to green) | W5 | 3 | The reinforcement-against-the-compiler loop producing a published Lumen-versus-Python number |
| 10 | Governance and evolution velocity | - | - | Already true; no pending gate |
| 11 | Debuggability | - | 2 | Deterministic time-travel replay landing and being CI-gated (the same Arc 2 tape as dimension 3's replay half) |
| 12 | Ecosystem breadth and hiring | W7 | 4 | Subsumption strategy, not a single gate to clear |
| 13 | Human familiarity today | W5 | 3 | The same reinforcement-against-the-compiler loop as dimension 9 |

## Where this sits

This document is the destination and the bets. `docs/ROADMAP_2036.md` is the plan of record that turns it into scheduled work: the arcs, their exit gates, and their kill criteria (`docs/ROADMAP.md` is the original phase plan it absorbs, now largely delivered). `docs/RISKS_AND_OPEN_PROBLEMS.md` is the list of things that can kill it. `docs/MANIFESTO.md` is the why. Read in that order, the project is honest with itself: a generation-ahead idea, a working bootstrap, and a decade of building between here and the vision above. `VISION_2036.md` is the competitive companion against the one incumbent, Python; `LANGUAGE_COMPARISON.md` widens the same honesty gate to the fourteen strongest rivals in the field, and its conclusion is the sharper form of this document's bet: Lumen does not out-max any single specialist on that specialist's own axis, it wins the integral, the only design that holds determinism, native lowering, machine-speed governance, and reward-environment structure together at once.
...
- Debuggability as a language feature becomes warm edit-to-diagnostic latency and the fraction of errors the compiler fixes itself. A confident fix the compiler applies costs the model zero output tokens and zero round-trips. That is the single highest-bandwidth lever in the whole design, and it is specified as the canonical Diagnostic and now ships: `lumen check --json` returns the structured diagnostic and `lumen fix` applies the confident ones over the warm daemon.
...
The earlier vision describes the compiler as an offline reward environment for training. The bandwidth thesis adds its inference-time twin: a long-lived `lumen serve` process that keeps the program model warm and answers check, fix, type, effects, callers, and ast as structured JSON over a local socket, with a span-edit protocol so the model sends a patch (tens of tokens) rather than a whole file (a thousand tokens) each round. Compile is already sub-millisecond in practice (0.5 ms cold, 1.5 ms incremental on 1,200 LOC, 794.1 kLOC/sec throughput against a 100 kLOC/sec target, `bench/DASHBOARD.md` Pillar B), so the bottleneck becomes the model's own latency, not the toolchain. The same daemon meters tokens-in, tokens-out, and rounds-to-green, feeding the authorship benchmark automatically.

### Prompt to B2B SaaS, through one mechanism

The concrete target is the kind of service in docs/b2b_saas_architecture.md: a multi-tenant quant platform with a submit-then-poll job contract over a queue. We reach it without importing a web framework. The single mechanism Lumen already ships, the capability as an ordinary typed parameter, is the one Http, Sql, Queue, Auth, Tenant, and Secret will express once Capabilities v1 lands (`docs/rfcs/0001-capabilities-v1.md`, status draft, PR #52): today the only capability that exists is `Console`. The purity claim, a function with no capability parameters is provably pure, is trivially true today only because `Console` is the sole capability; the sharper claim, a handler that omits the Tenant parameter provably cannot touch tenant data so multi-tenant isolation is a type error rather than a runtime audit, is the RFC's worked design example, not an implemented or CI-gated guarantee yet. A record auto-derives its deterministic JSON wire codec, closing the serialization dimension. The async job lifecycle is a stdlib sum type, and the worker body is the Result, match, and non-coercing ? operator that already landed. The correctness burden of a SaaS sits exactly where Lumen already has verified constructs, plus the capability set this section names as the next gate to clear.

### The tradeoffs we make on purpose

We are explicit about the costs, because the critics were right to press on them.

- We refuse to bake a vendor async skeleton (Pub/Sub topics, base64 envelopes, a submit/poll dispatcher) into the grammar.
- We refuse legacy-familiar syntax as a shortcut to first-try-compile-rate.
- We do not relabel cloud deploy as self-contained.
- The determinism contract taxes the fast path.

The full argument behind each of these four lives in `docs/LLM_BANDWIDTH_AND_PROMPT_TO_SAAS.md`.
...
One more honesty check, on "held-out" itself: the shards have to actually be sealed for that word to mean anything. Today the seal is `bench/promptgreen/`'s ten frozen tasks, hidden tests never shown to the author, proven so far by a scripted deterministic author, not yet a real model. The hundred-plus-task, published-log version that would make "held-out and metamorphic shards" airtight against a skeptic rather than a name for ten tasks is Arc 3 work (`docs/ROADMAP_2036.md`), not yet built. Until then, the seal is real but small, and we say so rather than borrow the credibility of a benchmark that does not exist yet.
