...
| 11 | Debuggability | pdb and tracebacks, post-hoc and at runtime | structured diagnostics, deterministic time-travel replay, compiler-applied fixes | `fix-application-rate`; replay coverage of recorded runs | Won on structure today (structured diagnostics and compiler-applied fixes ship); the replay half is designed, not built (the Arc 2 tape); Python leads on tooling maturity today |
